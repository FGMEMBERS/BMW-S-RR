...
var racelap = props.globals.getNode("/BMW-S-RR/race-lap");
var thissector = props.globals.getNode("/BMW-S-RR/this-sector");
...
var total_result = func{
	var maxlaps = getprop("/BMW-S-RR/"~pa~"/race-laps") or 0;
	var tr = 0;
    for(v = 0; v <= maxlaps; v +=1) {
		var actl = getprop("/BMW-S-RR/"~pa~"/lap["~v~"]/actual-time") or 0;
		tr += actl;
	}
...
var show_lap_and_sector_time = func{
	var show_it = getprop("/BMW-S-RR/show-race-times") or 0;
	var show_l_or_f = getprop("/BMW-S-RR/switch-last-fastest") or 0;
	...
	foreach(var s; sectors) { 
		...
		var lsts = getprop("/BMW-S-RR/"~pa~"/sector["~ln~"]/start-time") or 0;
		var sts  = getprop("/BMW-S-RR/"~pa~"/sector["~n~"]/start-time") or 0;
		var lt  = getprop("/BMW-S-RR/"~pa~"/sector["~n~"]/last-time") or 0;
		...
		if(thsec == n and racelap.getValue() > 0){
			...
			if(ct > 0){
				...
				setprop("BMW-S-RR/this-sector-time-sec", result[0]);
				setprop("BMW-S-RR/this-sector-time-min", result[1]);
			}
			if(ft > 0){
				var result = calc_time(ft);
				setprop("BMW-S-RR/fastest-sector-time-sec", result[0]);
				setprop("BMW-S-RR/fastest-sector-time-min", result[1]);
			}
			# only for mp
	...
	var fl = getprop("/BMW-S-RR/"~pa~"/fastest-lap") or 0;
	var ll = getprop("/BMW-S-RR/last-lap-time") or 0;
	...
	if(fl > 0){
		...
		setprop("BMW-S-RR/fastest-lap-time-sec", result[0]);
		setprop("BMW-S-RR/fastest-lap-time-min", result[1]);
		setprop("BMW-S-RR/fastest-lap-time-hour", result[2]);
	}
	if(lapt > 0){
		var result = calc_time(lapt);
		setprop("BMW-S-RR/this-lap-time-sec", result[0]);
		setprop("BMW-S-RR/this-lap-time-min", result[1]);
		setprop("BMW-S-RR/this-lap-time-hour", result[2]);
	}
	# only for mp
...
var find_marker = func{
	...
	if(getprop("/BMW-S-RR/"~pa~"/sector["~p~"]/fastest-time") == 0){
	...
	if(wp_distance < 260){
		var courserl = getprop("/BMW-S-RR/"~pa~"/race-laps") or 0;
	...
	if(wp_distance < 50){
		setprop("/BMW-S-RR/"~pa~"/sector["~thissector.getValue()~"]/start-time", getprop("/sim/time/elapsed-sec"));
		#print ("Sie ueberfuhren den "~thissector.getValue()~". Wegpunkt "~sectors[thissector.getValue()][3]);
		setprop("/BMW-S-RR/sector",sectors[thissector.getValue()][3]);

		var fastesttime = getprop("/BMW-S-RR/"~pa~"/sector["~ln~"]/fastest-time") or 0;
		var lastsectorstarttime = getprop("/BMW-S-RR/"~pa~"/sector["~ln~"]/start-time") or 0;
		...
		if(thissector.getValue() == 0){
		  ...
		  foreach(var ls; sectors) { 
		  	totallapresult += getprop("/BMW-S-RR/"~pa~"/sector["~n~"]/last-time") or 0;
			n+=1;
		  }
...
var clear_race_datas = func{
	...
	if(pa != nil and inrange != nil and inrange == 1){
		...
		laptime.setValue(0);
		racetime.setValue(0);
		var act = props.globals.getNode("/BMW-S-RR/"~pa);
		
		setprop("/BMW-S-RR/last-lap-time",0);
