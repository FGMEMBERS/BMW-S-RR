...
var virtual_racebikes_play =  func{
		if(wpvr.getValue() >= 0 and wpplay.getBoolValue()) {
			...
			if(replaywp <= wpvr.getValue()){
			 if (foo != 0) foo.remove();
			 foo = geo.put_model("/Aircraft/BMW-S-RR/VirtualRacer/BMW.xml", getprop("/virtual-racer/way/point["~replaywp~"]/lat"), getprop("/virtual-racer/way/point["~replaywp~"]/lon"), getprop("/virtual-racer/way/point["~replaywp~"]/elev-m") + 0.2, getprop("/virtual-racer/way/point["~replaywp~"]/hdg"), getprop("/virtual-racer/way/point["~replaywp~"]/pitch"), getprop("/virtual-racer/way/point["~replaywp~"]/roll")*80);
			  #print(replaywp);
			  replaywp += 1;
